...
#include "VulkanUIOverlay.hh"

namespace ost {
  UIOverlay::UIOverlay(ost::UIOverlayCreateInfo createInfo) noexcept {
  ...
  UIOverlay::~UIOverlay() noexcept {
    ...
    vkFreeCommandBuffers(createInfo.device->logicalDevice,
                         commandPool,
  ...
  void UIOverlay::prepareResources() noexcept {
    ...
    VkImageCreateInfo imageInfo = ost::initializers::imageCreateInfo();
    ...
    memAllocInfo.memoryTypeIndex =
      createInfo.device->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    // Image view
    VkImageViewCreateInfo viewInfo = ost::initializers::imageViewCreateInfo();
    ...
    VkSamplerCreateInfo samplerInfo = ost::initializers::samplerCreateInfo();
    ...
    VkCommandBufferAllocateInfo cmdBufAllocateInfo =
      ost::initializers::commandBufferAllocateInfo(commandPool,
    ...
    std::vector<VkDescriptorPoolSize> poolSizes = {
      ost::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1)
    };
    VkDescriptorPoolCreateInfo descriptorPoolInfo = ost::initializers::descriptorPoolCreateInfo(poolSizes, 2);
    VK_CHECK_RESULT(vkCreateDescriptorPool(createInfo.device->logicalDevice,
                                           &descriptorPoolInfo,
                                           nullptr,
                                           &descriptorPool));
    ...
    std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
      ost::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                                    VK_SHADER_STAGE_FRAGMENT_BIT,
                                                    0),
    };
    VkDescriptorSetLayoutCreateInfo
      descriptorLayout = ost::initializers::descriptorSetLayoutCreateInfo(setLayoutBindings);
    VK_CHECK_RESULT(vkCreateDescriptorSetLayout(createInfo.device->logicalDevice,
                                                &descriptorLayout,
                                                nullptr,
                                                &descriptorSetLayout));
    ...
    VkDescriptorSetAllocateInfo
      allocInfo = ost::initializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayout, 1);
    VK_CHECK_RESULT(vkAllocateDescriptorSets(createInfo.device->logicalDevice, &allocInfo, &descriptorSet));
    VkDescriptorImageInfo fontDescriptor = ost::initializers::descriptorImageInfo(
      sampler,
      fontView,
      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL
    );
    std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
      ost::initializers::writeDescriptorSet(descriptorSet,
                                            VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                                            0,
                                            &fontDescriptor)
    };
    vkUpdateDescriptorSets(createInfo.device->logicalDevice,
    ...
    VK_CHECK_RESULT(vkCreatePipelineCache(createInfo.device->logicalDevice,
    ...
    VkPushConstantRange
      pushConstantRange = ost::initializers::pushConstantRange(VK_SHADER_STAGE_VERTEX_BIT, sizeof(PushConstBlock), 0);
    VkPipelineLayoutCreateInfo
      pipelineLayoutCreateInfo = ost::initializers::pipelineLayoutCreateInfo(&descriptorSetLayout, 1);
    pipelineLayoutCreateInfo.pushConstantRangeCount = 1;
    pipelineLayoutCreateInfo.pPushConstantRanges = &pushConstantRange;
    VK_CHECK_RESULT(vkCreatePipelineLayout(createInfo.device->logicalDevice,
                                           &pipelineLayoutCreateInfo,
                                           nullptr,
                                           &pipelineLayout));

    // Command buffer execution fence
    VkFenceCreateInfo fenceCreateInfo = ost::initializers::fenceCreateInfo();
    VK_CHECK_RESULT(vkCreateFence(createInfo.device->logicalDevice, &fenceCreateInfo, nullptr, &fence));
  }

  /** Prepare a separate pipeline for the UI overlay rendering decoupled from the main application */
  void UIOverlay::preparePipeline() noexcept {
    // Setup graphics pipeline for UI rendering
    VkPipelineInputAssemblyStateCreateInfo inputAssemblyState =
      ost::initializers::pipelineInputAssemblyStateCreateInfo(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 0, VK_FALSE);

    VkPipelineRasterizationStateCreateInfo rasterizationState =
      ost::initializers::pipelineRasterizationStateCreateInfo(VK_POLYGON_MODE_FILL,
                                                              VK_CULL_MODE_NONE,
...
//    blendAttachmentState.alphaBlendOp = VK_BLEND_OP_ADD;
    ...
    VkPipelineColorBlendStateCreateInfo colorBlendState =
      ost::initializers::pipelineColorBlendStateCreateInfo(static_cast<u32>(blendStates.size()), blendStates.data());

    VkPipelineDepthStencilStateCreateInfo depthStencilState =
      ost::initializers::pipelineDepthStencilStateCreateInfo(VK_FALSE, VK_FALSE, VK_COMPARE_OP_LESS_OR_EQUAL);

    VkPipelineViewportStateCreateInfo viewportState =
      ost::initializers::pipelineViewportStateCreateInfo(1, 1, 0);

    VkPipelineMultisampleStateCreateInfo multisampleState =
      ost::initializers::pipelineMultisampleStateCreateInfo(createInfo.rasterizationSamples);
    ...
    VkPipelineDynamicStateCreateInfo dynamicState =
      ost::initializers::pipelineDynamicStateCreateInfo(dynamicStateEnables);

    VkGraphicsPipelineCreateInfo pipelineCreateInfo = ost::initializers::pipelineCreateInfo(pipelineLayout, renderPass);
    ...
    pipelineCreateInfo.stageCount = static_cast<u32>(createInfo.shaders.size());
    pipelineCreateInfo.pStages = createInfo.shaders.data();

    // Vertex bindings an attributes based on ImGui vertex definition
    std::vector<VkVertexInputBindingDescription> vertexInputBindings = {
      ost::initializers::vertexInputBindingDescription(0, sizeof(ImDrawVert), VK_VERTEX_INPUT_RATE_VERTEX),
    };
    std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
      ost::initializers::vertexInputAttributeDescription(0,
                                                         0,
    ...
    VkPipelineVertexInputStateCreateInfo vertexInputState = ost::initializers::pipelineVertexInputStateCreateInfo();
    ...
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(createInfo.device->logicalDevice,
                                              pipelineCache,
                                              1,
                                              &pipelineCreateInfo,
                                              nullptr,
  ...
  void UIOverlay::prepareRenderPass() noexcept {
  ...
  void UIOverlay::updateCommandBuffers() noexcept {
    ...
    renderPassBeginInfo.clearValueCount = static_cast<u32>(createInfo.clearValues.size());
    ...
    for ( usize i = 0; i < cmdBuffers.size(); ++i ) {
      ...
      VkViewport
        viewport = ost::initializers::viewport(ImGui::GetIO().DisplaySize.x, ImGui::GetIO().DisplaySize.y, 0.0f, 1.0f);
      vkCmdSetViewport(cmdBuffers[i], 0, 1, &viewport);

      VkRect2D scissor = ost::initializers::rect2D(( int32_t ) ImGui::GetIO().DisplaySize.x,
      ...
      pushConstBlock.scale = ost::Vec2(2.0f / io.DisplaySize.x, 2.0f / io.DisplaySize.y);
      pushConstBlock.translate = ost::Vec2(-1.0f);
      vkCmdPushConstants(cmdBuffers[i],
                         pipelineLayout,
      ...
      if ( createInfo.subpassCount > 1 ) {
        for ( u32 j = 1; j < createInfo.subpassCount; j++ ) {
  ...
  void UIOverlay::update() noexcept  {
    ...
    if ((vertexBuffer.buffer == VK_NULL_HANDLE) || (vertexCount != imDrawData->TotalVtxCount)) {
      vertexBuffer.unmap();
      vertexBuffer.destroy();
      VK_CHECK_RESULT(createInfo.device->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
    ...
    if ((indexBuffer.buffer == VK_NULL_HANDLE) || (indexCount < imDrawData->TotalIdxCount)) {
      indexBuffer.unmap();
      indexBuffer.destroy();
      VK_CHECK_RESULT(createInfo.device->createBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                                                      VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
                                                      &indexBuffer,
                                                      indexBufferSize));
  ...
  void UIOverlay::resize(u32 width, u32 height,const std::vector<VkFramebuffer>& framebuffers) noexcept {
  ...
  void UIOverlay::submit(VkQueue queue, u32 bufferindex, VkSubmitInfo submitInfo) noexcept {
  ...
  bool UIOverlay::header(const char *caption) noexcept{

    return ImGui::CollapsingHeader(caption, ImGuiTreeNodeFlags_DefaultOpen);
  }

  bool UIOverlay::button(const char *caption) noexcept{

    return ImGui::Button(caption);
  }

  void UIOverlay::text(const char *formatstr, ...) noexcept {
